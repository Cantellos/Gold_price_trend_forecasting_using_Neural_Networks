...
def train_model(model, train_X, train_y, val_X, val_y, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        loss.backward()
        # Clip dei gradienti per evitare l'esplosione dei gradienti nella GRU
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Training vs Validation Loss (GRU Model)")  # [MODIFICATO] Aggiornato il titolo
plt.legend()
plt.grid(True)
plt.show()
...
print(f"Final Test Loss (GRU): {test_loss:.4f}")  # [MODIFICATO] Aggiunto GRU nel messaggio
...
plt.xlabel("Time")
plt.ylabel("Price")
plt.title("Actual vs Predicted Gold Price using GRU (Test Set)")  # [MODIFICATO] Aggiornato il titolo
plt.legend()
plt.grid(True)
plt.show()